...
import { Shield, Clock, Banknote, MessageCircle, ChevronRight, ChevronLeft, ChevronDown, FileText, CheckCircle2, Star, Users, Award, Lock, RefreshCcw, Headphones, Heart, BadgeCheck, Scale, UserCheck, Building2, Sparkles, ArrowRight } from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    trackEvent('ab_variant_view', { variant_id: variant.id, page: 'home' });
  }, [variant.id]);

  const handleLineClick = (location) => {
    trackEvent('line_click', { variant_id: variant.id, location });
  };
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-12 md:py-20">
          <div className="text-center text-white mb-10">
            <p className="text-orange-100 text-sm md:text-base mb-3">もう、一人で悩まないで大丈夫です</p>
            <span className="inline-block px-5 py-2 bg-white text-orange-600 text-sm font-bold rounded-full mb-6 shadow-md">
              保育士専門の退職代行サービス
            </span>
            <h1 className="text-3xl md:text-5xl font-bold leading-tight mb-4 drop-shadow-md whitespace-pre-line">
              {variant.catchCopy}
            </h1>
            <p className="text-lg md:text-xl text-orange-100 max-w-2xl mx-auto mb-6">{variant.subCopy}</p>

            {/* Trust badges */}
            <div className="flex flex-wrap justify-center gap-3 mb-6">
              <div className="flex items-center gap-2 px-4 py-2 bg-white/95 rounded-full shadow-md">
                <Scale size={18} className="text-blue-500" />
                <span className="text-gray-800 font-bold text-sm">弁護士監修</span>
              </div>
              <div className="flex items-center gap-2 px-4 py-2 bg-white/95 rounded-full shadow-md">
